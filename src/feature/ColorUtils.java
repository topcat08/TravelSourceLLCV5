...
public class ColorUtils {
    ...
    public static Color getColorFromName(String name) {
        ArrayList<ColorName> acn = initColorList();
        for (ColorName cn : acn) {
            if (cn.name.equalsIgnoreCase(name)) {
                int r = cn.getR();
                int g = cn.getG();
                int b = cn.getB();
    ...
    public static Color getColorFromHex(String hexer) {
        String hex ="#"+hexer;
        Color c = null;
        try {
            c = new Color(Integer.valueOf(hex.substring(1, 3)), Integer.valueOf(hex.substring(3, 5)), Integer.valueOf(hex.substring(5, 7)));
            return c;
        }catch (Exception e) {
          //  e.printStackTrace();
        }
    ...
    /**
     * SubClass of screen.ColorUtils. In order to lookup color name
     * 
     * @author Xiaoxiao Li
     * 
     */
